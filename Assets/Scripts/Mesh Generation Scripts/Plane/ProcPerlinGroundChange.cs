...
public class ProcPerlinGroundChange : MonoBehaviour {
	...
	private void Start()
	{
		...
		Debug.Log("seedController: " + seedController.ToString());
		InitRefreshMesh();
	}
	...
	void Update()
	{
		...
		if (seedController.changeWorld)
		{
			//Get seeds from SeedController;
			priorSeed = seedController.priorSeed;
			seed = seedController.seed;
			Debug.Log("World changing!");
			Debug.Log("Initialising new Mesh: seed has changed from " + priorSeed + " to " + seed);
			InitRefreshMesh();
			seedController.changeWorld = false;
		}
	}
